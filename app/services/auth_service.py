...
def register_alumno(
    ...
    nombre: str,
    apellido: str,
    maestro_asignado: str,
    foto_url: str = None,
    dias: dict = None,
    franja_horaria: str = None,
    motivo_oracion: str = None
):
    """Registra un nuevo alumno creando persona + alumno + tarjeta en una transacción"""
    print(f"[debug] register_alumno called for {nombre} {apellido}")
    
    # Verificar que el maestro existe
    maestro_persona = db.query(Persona).filter(Persona.id_persona == maestro_asignado).first()
    if not maestro_persona:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="El maestro especificado no existe"
        )
    
    # Verificar que el maestro tiene registro en tabla maestros
    maestro = db.query(Maestro).filter(Maestro.id_persona == maestro_asignado).first()
    if not maestro:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="La persona especificada no es un maestro"
        )
    
    # Obtener rol de Alumno (id_rol=3)
    role_alumno = db.query(Role).filter(Role.id_rol == 3).first()
    if not role_alumno:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Rol 'Alumno' no configurado en el sistema"
        )
    
    # Obtener perfil de Alumno (id_perfil=3, siempre)
    perfil = db.query(Profile).filter(Profile.id_perfil == 3).first()
    if not perfil:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Perfil de Alumno (id_perfil=3) no configurado en el sistema"
        )
    
    try:
        # Crear persona sin email/password (alumno no tiene login propio)
        persona = Persona(
            auth_user_id=uuid.uuid4(),
            nombre=nombre,
            apellido=apellido,
        ...
        db.add(alumno)
        db.flush()  # Obtener id_alumno sin hacer commit
        
        # Crear tarjeta automáticamente vinculando al alumno con el maestro
        ...
        print(f"[debug] alumno registered: persona={persona.id_persona}, alumno={alumno.id_alumno}, tarjeta={tarjeta.id_tarjeta}")
        ...
        return {
            ...
            "name": f"{persona.nombre} {persona.apellido}",
            "maestro_asignado": str(maestro.id_maestro),
            "dias": alumno.dias,
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Error de integridad al registrar alumno."
        )
    except Exception as e:
        db.rollback()
        print(f"[error] failed registering alumno: {e}")
        traceback.print_exc()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error interno registrando alumno"
        )